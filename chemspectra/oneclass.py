...
class DDSimca(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, n_comps=2, alpha=0.05, gamma=0.05):
    ...
    def pred_acceptance_plot(self):
        ...
        p = figure(title="Acceptance Plot / Test Set", width=1100, height=550)
        p.add_tools(hover)
        p.xaxis.axis_label = "log(1 + h/h_0)"
        p.yaxis.axis_label = "log(1 + v/v_0)"
    ...
    def confusion_matrix(self, plot_cm='off', print_metrics='on'):
        ...
        self.metrics_list = [
            ...
            metrics.balanced_accuracy_score(cm_actual, cm_pred),
        ...
        if print_metrics == 'on':
            print(f"Accuracy: {self.metrics_list[0]}\n"
                  f"Balanced Accuracy: {self.metrics_list[1]}\n"
                  f"Precision: {self.metrics_list[2]}\n"
                  f"Sensitivity Recall: {self.metrics_list[3]}\n"
                  f"Specificity: {self.metrics_list[4]}\n"
                  f"F1_score: {self.metrics_list[5]}")
